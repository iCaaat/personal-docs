...
public class TokenSessionService {
    ...
    public TokenSessionService(JwtService jwtService, StringRedisTemplate redisTemplate) {
    ...
    public TokenPair rotate(String rawRefreshToken, AppUser user) {
        ...
        try {
            payload = jwtService.parse(rawRefreshToken);
        } catch (Exception exception) {
            throw unauthorized();
        }
        if (payload.type() != TokenType.REFRESH || !Objects.equals(payload.username(), user.getUsername()))
            throw unauthorized();
        if (!Boolean.TRUE.equals(redisTemplate.delete(refreshKey(payload.tokenId()))))
            throw unauthorized();
        return create(user);
    }
    ...
    private ResponseStatusException unauthorized() {
        return new ResponseStatusException(HttpStatus.UNAUTHORIZED, "刷新令牌无效或已过期");
    }
